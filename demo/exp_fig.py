-r# ~ from hpp.corbaserver.rbprm.talos_abstract import Robot
...
if pbName not in ['stairs', 'ground', 'debris', 'ramp_stairs2']:
...
afftool.visualiseAffordances('Support', v, [0.,0.,1.,0.4])
...
while run < MAX_RUN :
    ...
    # # v.client.gui.setVisibility("path_"+str(pathId)+"_root","ALWAYS_ON_TOP")
    # v.client.gui.setCurveLineWidth("path_"+str(pathId)+"_root",2.0)
    # v.client.gui.setCurveLineWidth("path_"+str(pathId)+"_root",2.0)
    ...
    if GUIDE:
        if CONTINUOUS:
            ...
            R, surfaces = getSurfacesFromGuide(rbprmBuilder,ps,afftool,None,step_size,INTERSECT)
    else:
...
# # color = list(colors[phase%len(colors)])+[1.]
# # color = list(colors[0])+[1.]
# color = [1,0,0,1]
#
# # v.client.gui.createGroup("Contact")
# ss = surfaces[phase]
...
#         p2[2]+= 0.001
#         name = "face"+str(i)+"."+str(k)
#         # v.client.gui.addTriangleFace(name,p2.tolist(),p1.tolist(),p0.tolist(),[0, 0.4470, 0.7410, 1.])
...
#     c = [np.average(p) for p in s]
#
#     v.client.gui.addCurve("normal"+str(j),[c[:3],(c+n*0.1).tolist()],color)
#     v.client.gui.setCurveLineWidth("normal"+str(j), 3.0)
#     v.client.gui.addToGroup("normal"+str(j), v.sceneName)
#
#
