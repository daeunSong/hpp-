...
if pbName == 'ground':
    ...
    rbprmBuilder.setJointBounds ("root_joint", [x_min, x_max, y_min, y_max, z_min + rbprmBuilder.ref_height, z_max + rbprmBuilder.ref_height])
...
if pbName in ['stairs_1', 'stairs_2']:
...
if pbName != 'ground':
    afftool.loadObstacleModel (packageName, "multicontact/daeun/"+pbName, "planning", vf,reduceSizes=[0.,0.,0.])
else:
...
def plotSurface (points, ax, plt,color_id = -1):
    ...
    else: ax.plot(xs,ys,zs, color = colors[color_id])
...
def draw_scene(surfaces, ax = None):
    ...
    for i, surfaces_phase in enumerate(surfaces): 
        for surface in surfaces_phase:
            plotSurface(surface, ax, plt, color_id%len(colors))
        color_id += 1
...
while run < MAX_RUN :   
    ...
    if not res_MI.success: # SL1M FAIL
        fail += 1
        continue
