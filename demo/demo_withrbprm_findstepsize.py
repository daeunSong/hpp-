...
# get an average number of candidates per phase  
def getAvgNumCandPerPhase (surfaces):
    total = 0; num =0
    for phase in surfaces:
        if len(phase) != 1:
            total += len(phase)
            num += 1
    return total/num
...
if __name__ == '__main__':
    ...
    fileName = "data/candidate/"+tp.pbName
    fileName_ps = "data/candidate/"+tp.pbName+"_ps" #phase and stepsizes
    ...
    while run < 10 :   
        ...
        avg = getAvgNumCandPerPhase(surfaces)
    ...
    data_ps = [s, s_i, s_f, s_if, p, p_i, p_f, p_if, avg_, avg_i, avg_f, avg_if]
