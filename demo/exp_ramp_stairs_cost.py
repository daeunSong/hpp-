...
while run < MAX_RUN :
    ...
    s_p0 = q_init[0:3]; init = footPosFromCOM(s_p0)
    ...
    g_p0 = q_goal[0:3]; goal = footPosFromCOM(g_p0)
    p_goal = ((goal[0]+goal[1])/2).tolist()
    p_goal[2] *= 2
    ...
    # res_MI = solveMIP_cost(pb, surfaces, ((goal[0]+goal[1])/2).tolist(), draw_scene,PLOT, CPP)
    ...
    ### solve
    res_MI_cost = solveMIP_cost(pb, surfaces, p_goal, draw_scene,PLOT, CPP)
    print(res_MI_cost)

    if not res_MI_cost.success: # SL1M FAIL
        fail1 += 1
