...
pbName = 'rubbles_bridge'#pbNames[I]
step_num = 29
...
# afftool.visualiseAffordances('Support', v, [0.,0.,1.,0.5])
...
while run < MAX_RUN :
    ...
    q_init = q_goal [::]
    q_init[-6:-3] = [0.2,0,0]
    q_goal[0:7] = [1.75,0.2,0.98,0,0,-0.7071,0.7071]
    q_goal[-6:-3] = [0,-0.2,0]
    ...
    ## stairs
    ps.setParameter("Kinodynamic/velocityBound",0.1)
    # ps.setParameter("Kinodynamic/accelerationBound",0.07)
    # ps.setParameter("Kinodynamic/velocityBound",0.3)
    ps.setParameter("Kinodynamic/accelerationBound",0.1)
    ps.resetGoalConfigs()
    ...
    v.client.gui.applyConfiguration("light", [2.5,0.,3.,0.,0.,0.,0.])
    v.client.gui.addToGroup("light", v.sceneName)
    v.client.gui.refresh()
    #
    # input("Press Enter to begin display ...")
    # pathLength = ps.pathLength(pathId)
    # i = 0;k = 0
    # for s in arange (0, pathLength, 0.01) :
    #     q = ps.configAtParam(pathId, s)
    #     if (k%10 == 0):
    #         if (s == 0):
    #             q_prev = q
    #         else:
    #             v.client.gui.addCurve("path_"+str(pathId)+str(s),[q_prev[:3],q[:3]],[0.9290, 0.6940, 0.1250, 1.0])
    #             v.client.gui.setCurveLineWidth("path_"+str(pathId)+str(s), 3.0)
    #             v.client.gui.addToGroup("path_"+str(pathId)+str(s), v.sceneName)
    #             v.client.gui.refresh()
    #         q_prev = q
    #
    #     v(q)
    ...
    #             displaySurfaceFromPoints(v,contact,colors[i%2])
    #         sleep(0.3)
    #         i += 1
    #     k+=1
    ...
    #     if norm(n) < 1e-6:
    #         n = cross(s[:,len(st[0])//3] - s[:,0], s[:,len(st[0])//2] - s[:,0])
    #     if n[2] <= 0.:
    #         for i in range(3):
    #             n[i] = -n[i]
    #     n /= norm(n)
    #     c = [np.average(p) for p in s]
    #     v.client.gui.addCurve("normal"+str(j),[c[:3],(c+n*0.1).tolist()],[0,0,1,1])
    #     v.client.gui.setCurveLineWidth("normal"+str(j), 3.0)
    #     v.client.gui.addToGroup("normal"+str(j), v.sceneName)

    pb = gen_pb(init, s_p0, R, surfaces); phase = len(pb["phaseData"])
    # res_MI = solveMIP(pb, surfaces, draw_scene, PLOT, CPP, 1)
...
cs.saveAsBinary("talos_ramp_stairs_no_ori.cs")
