...
rbprmBuilder = Robot ()
rbprmBuilder.setJointBounds ("root_joint", [-5,5,-5,5, 0.95, 1.0])#rbprmBuilder.ref_height,rbprmBuilder.ref_height])
...
q_init [0:3] = [-1.8, 0., 0.98]; v (q_init)#rbprmBuilder.ref_height]; v (q_init)
q_init[-6:-3] = [0.1,0,0]
q_goal = q_init [::]
q_goal [0:3] = [-0.9, 0.95, 0.98]; v (q_goal)#rbprmBuilder.ref_height]; v (q_goal) 
q_goal[-6:-3] = [0.1,0,0]
...
#q_init[-6:-3] = [0.,0,0]
q_goal [0:3] = [1.05, 0.95, 0.98]; v (q_init)#rbprmBuilder.ref_height]; v (q_goal) 
q_goal[-6:-3] = [0.1,0,0]
ps.setInitialConfig (q_init)
ps.addGoalConfig (q_goal)
v(q_goal)
...
q_init = rbprmBuilder.getCurrentConfig ();
q_init = q_goal[::]; v (q_init) 
q_goal [0:3] = [2.2, 0,  0.98]; v (q_goal)#rbprmBuilder.ref_height]; v (q_goal) 
q_goal[-6:-3] = [0.05,0,0]
ps.setInitialConfig (q_init)
ps.addGoalConfig (q_goal)
v(q_goal)
...
from mpcroc.test import contactSurfaces
surfaces = contactSurfaces(afftool) 
surfaces = sorted(surfaces) #sort the planes in order    
    
import pickle
with open('gp_slalom_debris_talos','wb') as f:
    pickle.dump(configs,f)  

with open('surf_slalom_debris','wb') as f:
